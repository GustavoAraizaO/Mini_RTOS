  ...
  if (RTOS_MAX_NUMBER_OF_TASKS > task_list.nTasks)
    {
      ...
      task_list.tasks[task_list.nTasks].sp = &(task_list.tasks[task_list.nTasks].stack[RTOS_STACK_SIZE - 1 - STACK_FRAME_SIZE]);	//stack is used bottoms up
...
static void dispatcher(task_switch_type_e type)
{
  ...
  {
      if (highest < task_list.tasks[index].priority && (task_list.tasks[index].state == S_READY || task_list.tasks[index].state == S_RUNNING))
	{
	  next_task= index;
...
FORCE_INLINE static void context_switch(task_switch_type_e type)
{
  ...
  if(!first_run){
	...
	task_list.tasks[task_list.current_task].sp = sp - 20;
      else
	task_list.tasks[task_list.current_task].sp = sp - 20;
...
static void activate_waiting_tasks()
{
  for (uint8_t task_to_check = 0; task_to_check <= task_list.nTasks; task_to_check ++){
